...
export interface NarrativeWorkspaceState {
  ...
  resolveDialogue?: (dialogueId: string) => Promise<ForgeGraph>
    ...
    devtools(
      (set, get) => {
        ...
        const ensureDialogueWithEvents = async (
          ...
          providedResolveDialogue?: (id: string) => Promise<ForgeGraph>
        ) => {
          const state = get()
        ...
        const setDialogueWithEvents = (id: string, dialogue: ForgeGraph) => {
