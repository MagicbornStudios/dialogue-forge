...
## What Lives in `src/shared/`
- **UI primitives**: Buttons, badges, cards, menus, and other composable UI building blocks live in `src/shared/ui/` (example: `Button` uses shared Tailwind + Radix patterns).
- **Shared utilities**: `src/shared/lib/utils.ts` provides helpers like `cn` for class name composition, used across Forge, Writer, and video tooling.
- **Shared types**: DOM event and keyboard constants live in `src/shared/types/index.ts` for consistent input handling across domains.
...
- **Reusable UI components** used by multiple domains → `src/shared/ui/`
- **Utilities** (formatting, helpers, small shared logic) → `src/shared/lib/`
- **Input constants or common data contracts** → `src/shared/types/`
