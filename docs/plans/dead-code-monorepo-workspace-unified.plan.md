...
- Update docs/architecture/workspace-editor-architecture.md to the full guide (subscriptions without an event bus, no draft slices, Video/Characters, anti-patterns, decision tree, file structure) as the single canonical workspace doc.
...
- **packages/video/src/index.ts** — VideoTemplateWorkspace export removed (done).
- **packages/video/src/workspace/VideoTemplateWorkspace.tsx** — Deleted (done).
- **packages/forge/src/lib/utils/forge-flow-helpers.ts** — Remove createGraphWithStartEnd (deprecated; no callers).
- **packages/forge/src/lib/utils/layout/layout.ts** — Remove applyDagreLayout and applyHierarchicalLayout (deprecated; no callers). Keep applyLayout and resolveNodeCollisions export.

**No code change, note in living doc only:**

- packages/shared/src/types/forge-graph.ts: speaker on node type is legacy (comment: deprecated, use characterId). Mention in living doc "Types: forge-graph node has legacy speaker; prefer characterId."
...
**Replace/update** docs/architecture/workspace-editor-architecture.md with the full Workspace Editor Architecture Guide (subscriptions without an event bus, no draft/commit layers, contracts, session store, editor shell, command pattern, modal management, auto-save, slice organization, communication and subscription flow, anti-patterns including subscription cleanup, decision tree, file structure reference, domain-specific notes for Forge, Writer, Video, Characters).
...
2. **Workspace architecture** — Update docs/architecture/workspace-editor-architecture.md to the full guide (subscriptions without EventSink, no draft layers, Video/Characters, anti-patterns, decision tree, file structure, platform/host subsection).
