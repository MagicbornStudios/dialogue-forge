...
## Visual Scene System (BabylonJS)

The execution strategy recommendation (Option B for MVP) remains valid and is **compatible** with the BabylonJS visual system.

Key points:
- The execution engine remains authoritative for **choices, conditions, variables, and routing**.
- The Babylon runtime is a **presentation layer** that applies **Scene Cues** emitted during node execution.
- MVP ships with **Behavior A**: scenes persist unless a node changes them; Player choices overlay the current scene.

See: `gameplayer-babylonjs.md` for the Babylon VN Stage Template, cue vocabulary, and storylet enter/exit policies.
