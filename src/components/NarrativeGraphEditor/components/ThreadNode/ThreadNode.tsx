...
export function ThreadNode({ data, selected }: NodeProps<NarrativeFlowNodeData>) {
  ...
  return (
    <div
      ...
      } ${isDimmed ? 'opacity-30' : ''} ${isInPath ? 'ring-2 ring-df-npc-selected/50' : ''}`}
    >
      <Handle
        type="target"
      ...
      <div className="px-3 py-2 border-b border-df-node-border flex items-center gap-2 bg-df-control-bg">
        <div className="h-8 w-8 rounded-full flex items-center justify-center text-df-npc-selected border-df-npc-selected bg-df-npc-header/30">
          <ScrollText size={16} />
        </div>
        <div className="flex-1 min-w-0">
          <div className="text-xs uppercase tracking-wide text-df-text-tertiary">Thread</div>
