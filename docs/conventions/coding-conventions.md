...
- Domain packages (forge, writer, video, characters) may import from shared and runtime only.
...
- src/index.ts is the public API surface.
...
- **Domain context:** Wrap workspace roots with `data-domain="forge"` | `"writer"` | `"ai"` | `"video"` | `"character"` so [src/styles/contexts.css](../../src/styles/contexts.css) applies. Domain accents come from `--color-df-domain-*` in [src/styles/themes.css](../../src/styles/themes.css).
