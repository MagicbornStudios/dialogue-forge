...
interface ThreadNodeContextMenuProps {
  x: number;
  y: number;
  nodeId: string;
  onEdit: () => void;
...
export function ThreadNodeContextMenu({
  x,
  y,
  nodeId,
  onEdit,
  ...
  return (
    <div className="fixed z-50" style={{ left: x, top: y }}>
      <div className="bg-df-elevated border border-df-player-border rounded-lg shadow-xl py-1 min-w-[180px]">
        ...
        >
          <Edit3 size={14} className="text-df-npc-selected" /> Edit Thread
        </button>
        <button
          onClick={() => {
            onAddAct();
          ...
          className="w-full px-4 py-2 text-sm text-left text-df-text-primary hover:bg-df-control-hover flex items-center gap-2"
        >
          <Plus size={14} className="text-df-player-selected" /> Add Act
        </button>
        <button
          onClick={onClose}
