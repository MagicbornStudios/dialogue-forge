...
export function ChapterNode({ data, selected }: NodeProps<NarrativeFlowNodeData>) {
  ...
  return (
    <div
      ...
      } ${isDimmed ? 'opacity-30' : ''} ${isInPath ? 'ring-2 ring-df-conditional-text/50' : ''}`}
    >
      <Handle
        type="target"
      ...
      <div className="px-3 py-2 border-b border-df-node-border flex items-center gap-2 bg-df-control-bg">
        <div className="h-8 w-8 rounded-full flex items-center justify-center text-df-conditional-text border-df-conditional-border bg-df-conditional-header/30">
          <BookOpen size={16} />
        </div>
        <div className="flex-1 min-w-0">
          <div className="text-xs uppercase tracking-wide text-df-text-tertiary">Chapter</div>
