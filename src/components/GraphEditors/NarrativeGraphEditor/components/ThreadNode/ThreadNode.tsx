...
import { ScrollText, Edit3, Plus } from 'lucide-react';
import {
  ContextMenu,
  ContextMenuContent,
  ContextMenuItem,
...
export function ThreadNode({ data, selected, id }: NodeProps<NarrativeFlowNodeData>) {
  const title = data.label || id;
  const summary = data.description;
  const isDimmed = data.isDimmed ?? false;
  const isInPath = data.isInPath ?? false;

  // Glow effect based on selection and path state - purple for thread
    ...
    <ContextMenu>
        ...
        >
          ...
          <div className="px-3 py-2 border-b border-df-node-border flex items-center gap-2 bg-violet-500/10">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-xs uppercase tracking-wide text-violet-500/70">Thread</div>
      ...
      <ContextMenuContent className="w-48">
        <ContextMenuItem onSelect={() => data.onEdit?.()}>
          <Edit3 size={14} className="mr-2 text-df-npc-selected" /> Edit Thread
        </ContextMenuItem>
        <ContextMenuItem onSelect={() => data.onAddAct?.()}>
