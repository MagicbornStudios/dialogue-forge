...
import { getInitialSelection } from './utils/forge-workspace-utils';
...
import { setupNarrativeWorkspaceSubscriptions } from './store/slices/subscriptions';
...
interface ForgeWorkspaceProps {
  ...
  resolveGraph?: (graphId: string) => Promise<ForgeGraph>;
}
...
export function ForgeWorkspace({
  ...
  if (!domainStoreRef.current) {
    ...
    domainStoreRef.current = createNarrativeWorkspaceStore(
      {
        initialThread,
        initialDialogue: projectGraph,
        flagSchema,
        gameState,
        resolveDialogue: resolveGraph,
      },
      eventSink
    );
...
function ForgeWorkspaceInner({
  ...
  const handleDialogueChange = useCallback((nextDialogue: ForgeGraph) => {
  ...
  return (
    ...
    >
      ...
      <PlayModal
        ...
        graph={pageDialogue ?? defaultDialogueTree ?? { id: 'empty', title: 'Empty', startNodeId: '', nodes: {} }}
        flagSchema={activeFlagSchema}
