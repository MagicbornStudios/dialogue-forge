...
import type { ForgeNodeType, NarrativeForgeNodeType } from '@magicborn/forge/types/forge-graph';
import { FORGE_NODE_TYPE, NARRATIVE_FORGE_NODE_TYPE } from '@magicborn/forge/types/forge-graph';
...
/**
 * Edge menu node type mapping
 * 
 * Maps node types to their available connection types for edge context menus.
 * Consolidated from duplicate edge context menu components.
 */
export const EDGE_MENU_NODE_TYPES = {
  PLAYER: AVAILABLE_STORYLET_NODE_TYPES.PLAYER_EDGE,
  CHARACTER: AVAILABLE_STORYLET_NODE_TYPES.CHARACTER_EDGE,
  CONDITIONAL: AVAILABLE_STORYLET_NODE_TYPES.CONDITIONAL_EDGE,
  STORYLET: AVAILABLE_STORYLET_NODE_TYPES.STORYLET_EDGE,
} as const;

/**
 * Edge drop menu node type mapping
 * 
 * Maps node types to their available connection types for edge drop menus.
 * These are slightly different from edge context menus - they're used when 
 * dragging from a node handle to create new connections.
 * Consolidated from duplicate EdgeDropMenu components.
 */
export const EDGE_DROP_MENU_NODE_TYPES = {
  PLAYER: [FORGE_NODE_TYPE.CHARACTER, FORGE_NODE_TYPE.CONDITIONAL],
  CHARACTER: [FORGE_NODE_TYPE.PLAYER, FORGE_NODE_TYPE.CONDITIONAL],
  CONDITIONAL: [FORGE_NODE_TYPE.CHARACTER, FORGE_NODE_TYPE.PLAYER],
  STORYLET: [], // Not implemented yet
} as const;
