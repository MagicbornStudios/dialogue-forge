 ...
 * Parse Yarn content into node blocks
 */
function parseYarnContent(yarnContent: string): import('./types').YarnNodeBlock[] {
  const blocks: import('./types').YarnNodeBlock[] = [];
 ...
 * - Otherwise = CHARACTER
 */
function determineNodeTypeFromYarn(block: import('./types').YarnNodeBlock): import('@/src/types/forge/forge-graph').ForgeNodeType | null {
  const hasOptions = block.lines.some(line => line.trim().startsWith('-> '));
  const hasConditionals = block.lines.some(line => 
    line.includes('<<if') || line.includes('<<elseif') || line.includes('<<else')
  );
  ...
  if (hasConditionals && !hasOptions) {
    ...
    const conditionalLines = block.lines.filter(line => 
      line.includes('<<if') || line.includes('<<elseif') || line.includes('<<else') || line.includes('<<endif')
    );
