...
import { ForgeChoice, ForgeNode, FORGE_NODE_TYPE } from '@magicborn/forge/types/forge-graph';
...
export const PlayerNode = React.memo(function PlayerNode({ data, selected }: NodeProps<PlayerNodeData>) {
  ...
  useEffect(() => {
    if (headerRef.current && choices.length > 0) {
      ...
      choices.forEach((_choice: ForgeChoice, idx: number) => {
...
{choice.setFlags && choice.setFlags.length > 0 && (
                    <div className="mt-1.5 flex flex-wrap gap-1">
                      {choice.setFlags.map(flagId => (
                        <FlagIndicator key={flagId} flagId={flagId} flagSchema={flagSchema} />
                      ))}
                    </div>
                  )}
                </div>
