...
export function ActNode({ data, selected }: NodeProps<NarrativeFlowNodeData>) {
  ...
  return (
    <div
      ...
      } ${isDimmed ? 'opacity-30' : ''} ${isInPath ? 'ring-2 ring-df-player-selected/50' : ''}`}
    >
      <Handle
        type="target"
      ...
      <div className="px-3 py-2 border-b border-df-node-border flex items-center gap-2 bg-df-control-bg">
        <div className="h-8 w-8 rounded-full flex items-center justify-center text-df-player-selected border-df-player-selected bg-df-player-header/30">
          <Layers size={16} />
        </div>
        <div className="flex-1 min-w-0">
          <div className="text-xs uppercase tracking-wide text-df-text-tertiary">Act</div>
